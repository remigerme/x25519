...
void small_curve_1() {
    ...
    // Checking ladder
    mpz_set_ui(m, 77);
    ladder(rop.x, m, p.x, a24, mod);
    assert(mpz_cmp_ui(rop.x, 8) == 0);
...
void small_curve_2() {
    ...
    // Checking xdbl
    xdbl(&rop, p, a24, mod);
    assert(mpz_cmp_ui(rop.x, 284) == 0);

    // Checking xadd
    xadd(&rop, rop, p, p, mod);
    assert(mpz_cmp_ui(rop.x, 759) == 0);

    // Checking ladder
    mpz_set_ui(m, 104);
    ladder(rop.x, m, p.x, a24, mod);
    assert(mpz_cmp_ui(rop.x, 810) == 0);

    mpz_set_ui(m, 947);
    ladder(rop.x, m, p.x, a24, mod);
    assert(mpz_cmp_ui(rop.x, 755) == 0);
